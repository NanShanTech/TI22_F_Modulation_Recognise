...
def test_am_timedomain_envelope():
    ...
    assert analytic_env.min() == 1 - MOD_AMP, f"包络最小值应为 1 - A_m = {1 - MOD_AMP}"
    assert analytic_env.max() == 1 + MOD_AMP, f"包络最大值应为 1 + A_m = {1 + MOD_AMP}"
...
def test_fm_modulation_freq_correct():
    ...
    assert correlation > 0.99, f"与正确 FM 的相关系数 {correlation:.4f} 应接近 1.0"
...
if __name__ == "__main__":
    ...
    plt.title("FM time domain first 500samps")
    plt.xlabel("time (μs)")
    plt.ylabel("Amplitude (V)")
